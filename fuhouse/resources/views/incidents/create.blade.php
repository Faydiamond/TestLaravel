...
<div class="container mt-4">
   ...
   <div class="row">
      <div class="col">
         <div class="card">
            <div class="card-body">
               <form action="{{ route('storeincident') }}" method="post">
                  @csrf
                  @method('POST')
                  <div class="row">
                     <div class="col-md-3">
                        <div class="form-group mt-1">
                           <select class="form-select" aria-label="Default select example"  name="reserve_id" id="reserve_id" required> 
                              ...
                              @foreach ($reserves as $reserve)
                                  <option value="{{ $reserve->id }}">{{ $reserve->booking_date}}</option>
                              @endforeach
                           </select>
                  ...
                  <div class="mt-3" >
                     ...
                     <a href="{{ route('incidents') }}" class="btn btn-secondary">Cancelar</a>
                  </div>
               </form>
               @if ($errors->any())
